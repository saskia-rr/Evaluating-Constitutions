...
class Conversation:
    ...
    @staticmethod
    def save_full_conversation_to_markdown(dialogue, folder_path, experiment, correct_diagnosis):
        os.makedirs(folder_path, exist_ok=True)
        
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{experiment}_{correct_diagnosis}_{timestamp}.md"
...
class ExperimentRunner:
    ...
    def run_experiment(doctor, patient, critic, moderator, max_turns, model, experiment_name, correct_diagnosis):
        ...
        if correct_diagnosis_reached:
            Conversation.save_full_conversation_to_markdown(dialogue, f"full_conversations/{model}/{experiment_name}/{correct_diagnosis}", experiment_name, correct_diagnosis)
...
def main():
    ...
    patient_instructions = ExperimentRunner.load_instructions('agent_clinic_sample_updated.json')
    ...
    for key in patient_instructions:
        experiments.append({
            "name": "best_practices_detailed_temp0",
